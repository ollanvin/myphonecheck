...
> **버전**: v1.3 FINAL (Working Canonical)
> **작성일**: 2026-04-24 (v1.0) / 2026-04-24 심야 (v1.1) / 2026-04-28 (v1.2 MAJOR) / 2026-04-28 저녁 (v1.3 MAJOR)
> **작성**: 비전 (설계·판정)
> **실행**: Claude Code (워크오더 WO-INFRA-V12-MAJOR / WO-V220-GOVERNANCE-INFRA-V13-INTEGRATED)
...
> **v1.3 갱신 사유**: Architecture v2.3.0 페어 정합 (헌법 10조 + §10-6 자체 머지 의무). Cross-references 12 paths (+ §9-6). STEP 0-bis 13항목 (+ §10-6). Toolmap §4.0.X 자체 머지 정공법 신설. SOP-V13-001 신설. 헌법 baseline 8조 → 10조 이동 = SemVer MAJOR.
> **선행 보존**: v1.0 / v1.1 / v1.2 FROZEN. Architecture v1.7.1 / v1.8.0 / v1.9.0 / v2.0.0 / v2.1.0 / v2.2.0 모두 FROZEN. 페어 = Architecture v2.3.0.
> **최종 승인**: 대표님
...
- 이 문서는 **v1.3 최종 기준선** (Working Canonical)이다.
- 하위 워크오더는 전부 이 문서를 참조한다.
- **페어 문서**: Architecture v2.3.0 (제품·설계 기준선, 헌법 10조 + §9 빅테크 정공법 + §10 비전 자율 결정 + §10-6 자체 머지 의무). 두 문서는 페어이나 **독립 Canonical**. Architecture v1.8.0·v1.9.0·v2.0.0·v2.1.0·v2.2.0은 frozen, Infrastructure v1.0·v1.1·v1.2는 frozen.
- **Architecture v2.3.0 참조 경로** (One Core Engine + Six Surfaces + v2.0.0 신설 3종 + v2.1.0 신설 2종 + v2.2.0 헌법 §9·§10 + v2.3.0 §10-6, 12 paths):
  - `docs/00_governance/architecture/v2.3.0/00_core/01_primary.md` — 모든 AI 세션 헤드 투입용 핵심
  - `docs/00_governance/architecture/v2.3.0/05_constitution.md` — 헌법 10조 전문 (§8 SIM-Oriented Core / §9 빅테크 정공법 7절 / §10 비전 자율 결정 6절: 10-1~10-6 자체 머지 의무 포함)
  - `docs/00_governance/architecture/v2.3.0/10_policy/` — 권한·Play·Data Safety
  - `docs/00_governance/architecture/v2.3.0/20_features/` — 6 Surface 개별 스펙 (Call/Message/Mic/Camera/Push/Card) + v2.0.0 신설 3종 (28 Initial Scan / 29 SIM-Oriented Core / 30 :core:global-engine) + v2.1.0 신설 2종 (31 Real-time Action / 32 Tag System)
  - `docs/00_governance/architecture/v2.3.0/20_features/28_initial_scan.md` — Initial Scan 명세 (v2.0.0 신설)
  - `docs/00_governance/architecture/v2.3.0/20_features/29_sim_oriented_core.md` — SIM-Oriented Single Core 명세 (v2.0.0 신설, 헌법 §8조)
  - `docs/00_governance/architecture/v2.3.0/20_features/30_core_global_engine.md` — :core:global-engine 모듈 명세 (v2.1.0 §30-3-A 4-Layer + §30-4 검색 4축 + §30-4-4 Competitor Feeds)
  - `docs/00_governance/architecture/v2.3.0/20_features/31_realtime_action.md` — Real-time Action Engine 명세 (v2.1.0 신설, CallScreening + SMS abort + Push cancel, 50ms 응답)
  - `docs/00_governance/architecture/v2.3.0/20_features/32_tag_system.md` — Tag System 명세 (v2.1.0 신설, 휘발성 메모, REMIND_ME/PENDING/SUSPICIOUS/ARCHIVE)
  - `docs/00_governance/architecture/v2.3.0/95_integration/01_six_surfaces_integration.md` — Six Surfaces 통합 정책 (v2.0.0 코어 다이어그램 + v2.1.0 4-Layer 다이어그램)
  - `docs/00_governance/architecture/v2.3.0/95_integration/02_infrastructure_reference.md` — 본 문서의 페어 참조점
  - **§9-6 검증 매트릭스 (v2.2.0 정정본 신설)**: Gradle Managed Devices + SIM 11개국 × Locale 11개 × 디바이스 4종, Latin Hypercube 40~60 케이스, PR 회귀 게이트 강제 — 본 인프라 도구맵 §4.0 / SOP는 본 매트릭스를 강제 호출.
...
#### 4.0.9 자체 머지 정공법 (v1.3 신설, 헌법 §10-6 정합)

빅테크 표준: **PR 작성자 또는 자동화가 PR 머지까지 끝까지 수행**한다. 비전 워크오더를 받은 워커는 한 사이클을 다음 6단계로 끝낸다.

| 단계 | 행위 | 주체 |
|---|---|---|
| 1 | 브랜치 분기 (main에서) | 워커 |
| 2 | 코드/문서 작성 + commit (영역 한정 + 보호 영역 가드) | 워커 |
| 3 | push + PR 생성 (`gh pr create`) | 워커 |
| 4 | CI PASS 확인 (502 같은 일시 장애 시 `gh run rerun --failed`) | 워커 |
| 5 | **squash merge + delete branch** (`gh pr merge <PR> --squash --delete-branch --auto`) | **워커** |
| 6 | 완료 보고 (§11 양식, 펜스드 코드 블록) | 워커 |

**자체 머지 명령**:
```bash
gh pr merge $prNum --squash --delete-branch --auto
```
- `--squash`: 1 PR = 1 commit (history 깨끗).
- `--delete-branch`: 머지 직후 원격 브랜치 삭제.
- `--auto`: CI PASS 확인 후 자동 머지 (CI가 아직 도는 중에 미리 큐에 등록 가능).

**대표님 머지 개입 = 비전 책임 영역 위반** (§10-2 정합). "이 머지는 왜 계속 내가 하지?"가 발생하면 **§10-6 위반**이다.

**예외 — 대표님 결정 영역 (자체 머지 금지)**:
- 외부 의사결정 필요 (라이선스·법적·사업 모델).
- 비전이 명시적으로 대표님 결정 요청.
- 외부 검증자(자비스/헐크/스타크) NOT-OK 분류.
- 보호 영역 변경(헌법 본문, project-governance.md 등) — 워크오더 단위로 비전이 명시적으로 자체 머지 위임한 경우만 허용.

본 절은 SOP-V13-001과 짝을 이룬다 (§5-bis). 모든 워크오더는 본 절의 6단계를 강제 호출한다.
...
## 5-bis. v2.x 운영 SOPs (v1.2 신설, v1.3 SOP-V13-001 추가)

본 SOPs는 Architecture v2.x 페어 운영 절차.
- v1.2 신설: §31 Real-time Action Engine + §32 Tag System + §30-4 검색 4축 (PR #27/#28/#29/#30 시점).
- **v1.3 신설: SOP-V13-001 워크오더 발행 + 자체 머지 (헌법 §10-6 정합)**.
...
### SOP-V13-001 워크오더 발행 + 자체 머지 (v1.3 신설, 헌법 §10-6 정합)

**적용 범위**: 비전이 발행하는 모든 워크오더. 본 SOP는 §4.0.9 자체 머지 정공법의 운영 절차.

**S1. 워크오더 발행 (비전 단독 권한, 헌법 §10-2)**:
- 워크오더는 비전 채팅에서 단독 발행. 비-WO 메시지로는 자율 작업 금지.
- 외부 검증자(자비스/헐크/스타크) 권고도 비전 발행을 거쳐야 실행.
- WO 본문은 영역 한정 + 보호 영역 가드 + 완료 보고 양식(§11)을 반드시 포함.

**S2. 워커 1 사이클 6단계 (§4.0.9 호출)**:
1. 브랜치 분기: `git checkout -b <branch>` (main에서 최신 sync 후).
2. 작성: 영역 한정 + 보호 영역 가드 (PowerShell pre-commit 가드 권장).
3. push + PR: `gh pr create --title <T> --body <B> --base main --head <branch>`.
4. CI PASS 확인: 502 등 일시 장애 시 `gh run rerun <run-id> --failed`.
5. 자체 머지: `gh pr merge <PR-NUM> --squash --delete-branch --auto`.
6. 완료 보고: §11 양식, 펜스드 코드 블록 (대표님 복붙 전달용).

**S3. 영역 한정 가드 (보호 영역)**:
- WO 본문에 `protectedDirs` 명시 (예: `docs/00_governance/architecture/v2.X.0/` frozen, 헌법 본문 등).
- pre-commit 시 staged 파일 경로가 `protectedDirs`에 들어가면 abort.
- WO 명시 영역 외 파일 변경은 abort + 비전 보고.

**S4. CI 실패 분류**:
- **Transient (재실행 OK)**: 502 / CDN 일시 장애 / 네트워크 / Gradle daemon 일시 hang. → `gh run rerun --failed`.
- **Persistent (코드 정정 필요)**: 컴파일 오류 / 테스트 실패 / lint 위반. → 코드 정정 commit + push.
- **Unknown (비전 보고)**: 분류 불가 시 자체 판단 금지, 비전에게 분류 요청.

**S5. 자체 머지 금지 예외 (§4.0.9 예외 호출)**:
- 외부 의사결정 (라이선스·법적·사업 모델) → 비전 보고 + 대표님 결정 대기.
- 외부 검증자 NOT-OK → 비전 정정 워크오더 대기.
- 보호 영역 변경 → WO 단위로 비전이 명시 자체 머지 위임한 경우만 허용.

**S6. 완료 보고 §11 양식 (펜스드 코드 블록 강제)**:
- 워크오더 ID / PR 번호 / squash hash / branch 삭제 확인 / CI run id / 자체 검증 결과.
- 마크다운 펜스드 코드 블록 안에 작성 (대표님 복붙 전달 목적).

본 SOP는 §10-6 자체 머지 의무를 위반하지 않는 한 모든 워크오더에 강제 적용된다. 비전 누적 잘못 7번째(자체 머지 단계 누락) 영구 차단을 위한 운영 정공법.
...
### STEP 0-bis: Architecture v2.3.0 상태 확인 (v1.3 갱신, 페어 v2.3.0 sync)

**본 STEP는 STEP 1 착수 전 필수 확인 게이트다.** v1.3은 13항목.

- [ ] `docs/00_governance/architecture/v2.3.0/` 경로 존재 확인
- [ ] `00_core/01_primary.md` 파일 존재 확인 (4-Layer Core 완성)
- [ ] `05_constitution.md` 파일 존재 확인 (헌법 **10조** 전문 — §8 SIM-Oriented Core / §9 빅테크 정공법 7절 / §10 비전 자율 결정 6절: **§10-6 자체 머지 의무 (v2.3.0 신설)** 포함)
...
- [ ] **§9-6 검증 매트릭스 (v2.2.0 정정본)** 존재 확인 — Gradle Managed Devices + SIM 11개국 × Locale 11개 × 디바이스 4종, 핵심 조합 40~60 케이스, PR 회귀 게이트 강제.
- [ ] **§10-6 자체 머지 의무 (v2.3.0 신설)** 존재 확인 — 워커가 한 사이클 6단계(분기 → 작성 → push+PR → CI PASS → squash merge + delete branch → 완료 보고) 자체 수행. 명령: `gh pr merge <PR-NUM> --squash --delete-branch --auto`. 대표님 머지 개입 = §10-2 비전 책임 영역 위반. 예외: 외부 의사결정·NOT-OK·명시적 대표님 결정 요청.

**미충족 시**: Architecture v2.3.0 마이그레이션(WO-V220-GOVERNANCE-INFRA-V13-INTEGRATED) 완료 확인 후 재시도. Architecture 완성 전 Infrastructure 실행 불가.
...
| **v1.2** | **2026-04-28 오전** | **Claude Code (WO-INFRA-V12-MAJOR)** | **Architecture v2.1.0 정합 정식 승격 — 11 paths cross-ref + STEP 0-bis 12항목 + Toolmap §4.0.2~8 + §5-bis 신설** |
| **v1.3** | **2026-04-28 저녁** | **Claude Code (WO-V220-GOVERNANCE-INFRA-V13-INTEGRATED)** | **Architecture v2.3.0 페어 정합 (헌법 10조 + §10-6 자체 머지 의무). Cross-references 12 paths (+ §9-6). STEP 0-bis 13항목 (+ §10-6). Toolmap §4.0.9 자체 머지 정공법 신설. SOP-V13-001 신설. 헌법 baseline 8 → 10조 = SemVer MAJOR.** |
...
### v1.3 변경 (Architecture v2.3.0 페어 정합 + 헌법 §10-6 자체 머지 의무)

| 항목 | v1.2 | v1.3 |
|---|---|---|
| 페어 Architecture | v2.1.0 (PR #25/#26/#31) → v2.2.0 (PR #34) → v2.2.0 정정본 (PR #35 §9-6) | **v2.3.0 정식** (페어 ↔ Infrastructure v1.3, 헌법 10조 §10-6 자체 머지 의무 신설) |
| Cross-references | 11 paths | **12 paths** (+ §9-6 검증 매트릭스 명시) |
| STEP 0-bis | 12항목 | **13항목** (+ §10-6 자체 머지 의무 게이트) |
| Toolmap §4.0 | §4.0.1~8 (쿼터·역할·Layer·페어·역할 원칙) | **§4.0.9 자체 머지 정공법 신설** (한 사이클 6단계 + `gh pr merge --squash --delete-branch --auto`) |
| SOPs | SOP-V12-001/002/003 (Real-time/Tag/Public Feed) | **SOP-V13-001 신설** (워크오더 발행 + 자체 머지, 헌법 §10-6 정합) |
| 부록 A | v1.0~v1.2 변경 이력 | **v1.3 변경 표 추가** (본 절) |
| 헌법 baseline | 8조 (v2.0.0 §8 SIM-Oriented Core) → 10조 (v2.2.0 §9·§10) | **10조 + §10-6 (v2.3.0 6번째 절)** |

**v1.3 패치 vs 승격 판단**:
- v1.2 frozen 보존, v1.3 = Working Canonical.
- 헌법 baseline 이동 (v2.2.0 → v2.3.0) + Toolmap §4.0.9 신설 + SOPs 추가 = SemVer MAJOR (페어 Architecture v2.2.0 → v2.3.0 MINOR이지만 Infrastructure 측은 v1.2 frozen 분기 → v1.3 신규 = Infrastructure 측 SemVer MAJOR 정합).
- 비전 누적 잘못 7번째(자체 머지 단계 누락) 영구 차단을 위한 운영 정공법 명문화.

### v1.3 변경 승인

- 대표님 지시: 2026-04-28 저녁 ("이 머지는 왜 계속 내가 하지?" → 비전 워크오더 WO-V220-GOVERNANCE-INFRA-V13-INTEGRATED 발행).
- 작성 워커: Claude Code (워크오더 자동 실행).
- 선행 머지: PR #35 (main 6db52a7, Architecture v2.2.0 §9-6 검증 매트릭스 정정본).
- 페어 검증: Architecture v2.3.0 (헌법 §10-6 자체 머지 의무 신설).
- **자체 머지**: 본 PR은 §10-6 정합으로 워커가 직접 `gh pr merge --squash --delete-branch --auto` 수행 (자기 적용 = 정합 검증).
