...
- **페어 문서**: Architecture v1.8.0 (제품·설계 기준선). 두 문서는 페어이나 **독립 Canonical**.
- **Architecture v1.8.0 참조 경로** (4-Layer 분리 구조):
  - `docs/00_governance/architecture/v1.8.0/00_core/01_primary.md` — 모든 AI 세션 헤드 투입용 핵심
  - `docs/00_governance/architecture/v1.8.0/05_constitution.md` — 헌법 7조 전문
  - `docs/00_governance/architecture/v1.8.0/10_policy/` — 권한·Play·Data Safety
  - `docs/00_governance/architecture/v1.8.0/20_features/` — 4 Surface 개별 스펙
  - `docs/00_governance/architecture/v1.8.0/95_integration/02_infrastructure_reference.md` — 본 문서의 페어 참조점
...
> **Architecture v1.8.0 정합 (v1.1 추가)**: 본 §1의 헌법 원칙은 Architecture v1.8.0 `05_constitution.md`와 완전 정합한다. 헌법 원본은 Architecture 측이며, 본 문서는 인프라 운영 관점에서 핵심 조항만 인용한다. 헌법 7조 전문과 상세 해석 규칙은 Architecture v1.8.0 `00_core/01_primary.md` §2 (조항명)와 `00_core/02_secondary.md` §1 (해석 규칙) 참조.
...
### STEP 0-bis: Architecture v1.8.0 상태 확인 (v1.1 신설)

**본 STEP는 STEP 1 착수 전 필수 확인 게이트다.**

- [ ] `docs/00_governance/architecture/v1.8.0/` 경로 존재 확인
...
- [ ] `20_features/` 디렉토리 존재 확인 (4 Surface 스펙)
- [ ] `95_integration/02_infrastructure_reference.md` 파일 존재 확인 (본 문서의 페어 참조점)

**미충족 시**: Architecture v1.8.0 마이그레이션(WO-V180-MIGRATE-003) 완료 확인 후 재시도. Architecture 완성 전 Infrastructure 실행 불가.
