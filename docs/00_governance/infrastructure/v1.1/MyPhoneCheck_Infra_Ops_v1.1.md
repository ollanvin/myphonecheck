...
> **버전**: v1.1 FINAL
> **작성일**: 2026-04-24 (v1.0) / 2026-04-24 심야 (v1.1)
> **작성**: 비전 (설계·판정)
...
- 이 문서는 **v1.1 최종 기준선**이다.
- 하위 워크오더는 전부 이 문서를 참조한다.
- **페어 문서**: Architecture v2.0.0 (제품·설계 기준선, One Core Engine + SIM-Oriented + Initial Scan). 두 문서는 페어이나 **독립 Canonical**. v1.8.0·v1.9.0은 frozen.
- **Architecture v2.0.0 참조 경로** (One Core Engine + Six Surfaces + v2.0.0 신설 3종, 9 paths):
  - `docs/00_governance/architecture/v2.0.0/00_core/01_primary.md` — 모든 AI 세션 헤드 투입용 핵심
  - `docs/00_governance/architecture/v2.0.0/05_constitution.md` — 헌법 8조 전문 (제8조 SIM-Oriented Single Core 신설)
  - `docs/00_governance/architecture/v2.0.0/10_policy/` — 권한·Play·Data Safety
  - `docs/00_governance/architecture/v2.0.0/20_features/` — 6 Surface 개별 스펙 (Call/Message/Mic/Camera/Push/Card) + v2.0.0 신설 3종 (28 Initial Scan / 29 SIM-Oriented Core / 30 :core:global-engine)
  - `docs/00_governance/architecture/v2.0.0/20_features/28_initial_scan.md` — Initial Scan 명세 (v2.0.0 신설)
  - `docs/00_governance/architecture/v2.0.0/20_features/29_sim_oriented_core.md` — SIM-Oriented Single Core 명세 (v2.0.0 신설, 헌법 §8조)
  - `docs/00_governance/architecture/v2.0.0/20_features/30_core_global_engine.md` — :core:global-engine 모듈 명세 (v2.0.0 신설, One Core Engine)
  - `docs/00_governance/architecture/v2.0.0/95_integration/01_six_surfaces_integration.md` — Six Surfaces 통합 정책 (v2.0.0 코어 다이어그램)
  - `docs/00_governance/architecture/v2.0.0/95_integration/02_infrastructure_reference.md` — 본 문서의 페어 참조점
...
#### 4.0.2 도구별 주·보조 역할 (v1.1 신설)

| 도구 | 주 역할 | 보조 역할 |
|---|---|---|
| Cursor | 실행 (구현·자동화) | — |
| 코웍 | 실행 (문서·GitHub) | — |
| Codex CLI | 감사 (미시·본문 모순 탐지) | 실행 (긴급 시) |
| Claude Code | 감사 (거시·실제 명령 검증) | 실행 (장시간 주행 필요 시) |
| 비전 채팅 | 설계·판정 | 감사 통합 |
...
#### 4.0.4 역할 원칙 (기존 §2.3 감사 대칭 규칙 확장)

v1.0 §2.3 "감사 대칭: Cursor=비전, Claude Code=Codex"은 v1.1에서 다음으로 확장된다:

- **기계적 재배치·구현 작업**: Cursor + 코웍 실행 → Codex + Claude Code 감사
- **창작 포함 업데이트**: Cursor 단독 실행 → Codex + Claude Code 감사
- **Surface 개별 구현**: Cursor + Claude Code 실행 → Codex 감사
- **동일 작업에 Cursor + Claude Code 병용 금지** 원칙 유지 (v1.0 계승)
...
### STEP 0-bis: Architecture v2.0.0 상태 확인 (v1.1 신설, v2.0.0 sync)

**본 STEP는 STEP 1 착수 전 필수 확인 게이트다.**

- [ ] `docs/00_governance/architecture/v2.0.0/` 경로 존재 확인
- [ ] `00_core/01_primary.md` 파일 존재 확인 (4-Layer Core 완성)
- [ ] `05_constitution.md` 파일 존재 확인 (헌법 **8조** 전문, 제8조 SIM-Oriented Single Core 신설)
- [ ] `10_policy/` 디렉토리 존재 확인 (권한·Play·Data Safety)
- [ ] `20_features/` 디렉토리 존재 확인 (Six Surface 스펙 + v2.0.0 신설 3종)
...
- [ ] `20_features/30_core_global_engine.md` 파일 존재 확인 (:core:global-engine, v2.0.0 신설, One Core Engine)
- [ ] `95_integration/01_six_surfaces_integration.md` 파일 존재 확인 (Six Surfaces 통합 정책 + v2.0.0 코어 다이어그램)
- [ ] `95_integration/02_infrastructure_reference.md` 파일 존재 확인 (본 문서의 페어 참조점)

**미충족 시**: Architecture v2.0.0 마이그레이션(WO-V200-MAJOR-001, PR #15 squash `2dd2bc6`) 완료 확인 후 재시도. Architecture 완성 전 Infrastructure 실행 불가.
