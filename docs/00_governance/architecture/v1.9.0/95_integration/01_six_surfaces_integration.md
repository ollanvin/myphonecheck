...
> **v1.9.0 Surface 정의 정정 (§17-1 참조)**: Surface는 **사용자 가치 추출 단위 (Value Extraction Layer)**이며 Decision Engine 공유는 위협 평가 Surface (CallCheck / MessageCheck) 한정. PushCheck는 v1.9.0 Stage 1 시점 **규칙 기반 휴지통** (앱/채널 차단 규칙)으로 작동하며, Decision Engine 통합은 **Stage 2+ 후속 작업**으로 위임. MicCheck / CameraCheck (PackageManager 직접) 및 CardCheck (SMS/Push 재활용)는 자체 로직.

## 36-1. 통합 엔진 (One Engine)

위협 평가 Surface (CallCheck·MessageCheck) 는 **단일 DecisionEngineContract**를 공유한다. PushCheck는 v1.9.0 Stage 1에서 규칙 기반(앱/채널 차단)으로 작동하며 DE 통합은 후속이다. MicCheck·CameraCheck·CardCheck는 같은 헌법을 따르되 별도 로컬 로직으로 가치를 추출한다.
