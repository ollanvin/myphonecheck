...
**원본 출처**: v1.7.1 §3 (471–546)
**v1.9.0 Layer**: Core
**의존**: `00_core/01_primary.md`
**변경 이력**: v1.8.0 이관본을 기반으로 v1.9.0 MAJOR 승격(Six Surfaces, Surface 정의 정정) 반영.
**파일 경로**: `docs/00_governance/architecture/v1.9.0/06_product_design/02_golden_egg.md`
...
황금알은 Surface가 달라도 **동일 가치 규격**으로 산출된다. v1.6에서 Four Surfaces로 확장된 원칙은 v1.9.0에서 **Six Surfaces**로 승격되며, 이후 Surface 추가 시에도 동일 원칙을 유지한다.
...
| PushCheck | NotificationListener 메타데이터 + 사용자 차단 규칙 | `decisionEngine.evaluate(notificationSource)` + 격리 로직 | PushRisk (4속성) + Trash Item |
| CardCheck | 카드 결제 SMS/Push | 자체 파서 (엔진 미사용) | CardSummary (월별 집계 + 4속성 카드 셸) |

**핵심**: Surface는 **Value Extraction Layer (사용자 가치 추출 단위)**이며, 반드시 같은 엔진을 호출할 필요는 없다. Decision Engine은 CallCheck·MessageCheck·PushCheck 같은 **위협 평가 Surface**가 공유하고, MicCheck·CameraCheck는 PackageManager 직접 스캔, CardCheck는 SMS/Push 재활용 파서로 가치를 추출한다. 앱 평판·CVE 감시는 여전히 후행 Surface `AppSecurityWatch`에 남는다.

**Surface 정의 (v1.9.0 정정)**: Surface = 사용자 가치 추출 단위. 공통점은 "사용자에게 직접 가치를 보여주는 표면"이며, 내부 구현은 엔진 공유형/권한 감사형/거래 추출형으로 나뉜다.
...
단일 카드 `FourAttributeCard`에 4속성을 모두 담아 **Six Surfaces 간 일관된 UX**를 보장한다.
