...
**원본 출처**: v1.7.1 §21 (2553–2594)
**v1.9.0 Layer**: Verification
**의존**: `00_core/01_primary.md`
**변경 이력**: v1.9.0 Six Surfaces 단계 문구 반영.
**파일 경로**: `docs/00_governance/architecture/v1.9.0/80_verification/03_open_issues.md`
...
| 12 | Six Surfaces 우선순위 | 모두 병행 vs 순차 | Phase 1(Call) → Phase 2(Message) → Phase 3(Mic/Camera) → Phase 4(Push 정식, Stage 1-001 완료) → Phase 5(Card 신설, Stage 1-002 예정) 순차 |
...
| **15** | **AppSecurityWatch Surface** (Patch 31) | MicCheck/CameraCheck에 포함됐던 CVE·침해 감시 → 단순 관리자 축소(Patch 30)로 분리 필요 | **§17-3에 후행 Surface로 유지, PushCheck·CardCheck 정식화 이후 별도 워크오더 진입** |
...
| Phase 3.5 | PushCheck | 스팸 지정 앱 알림 격리 + 휴지통 복원 강조 |
| Phase 4.5 | CardCheck | 카드 결제 고정 카드·고액 사용 내역 강조 |
| Phase 후행 | AppSecurityWatch | 동일 규칙 자동 계승 |
