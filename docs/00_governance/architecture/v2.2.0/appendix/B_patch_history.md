...
| **PATCH-54** | **헌법 §9 빅테크 정공법 신설** | **05_constitution.md** | **신규 조항** | **9-1 언어·번역(values-xx 수동 추가 금지, OS 위임) + 9-2 운영 모델(외부 영업·계약 0) + 9-3 출시 정책(글로벌 단일, iOS·Android 동등) + 9-4 코드 정공법(분기 0, ICU·ISO만) + 9-5 위반 시 PR 차단 + 9-6 비고. 비전 누적 위반(시장 분리·번역·계약·iOS 분리) 영구 차단.** |
...
**MAJOR 승격 기록 (2026-04-28 v2.2.0)**: Patch 54~56 반영 시점에 **v2.1.0 → v2.2.0 MAJOR 에스컬레이션**. 사유: 헌법 §9 빅테크 정공법 + §10 비전 자율 결정 신설(헌법 본문 변경 = SemVer MAJOR). 비전이 v2.0.0~v2.1.0 시점까지 메모리 학습이 있었음에도 시장 분리·번역·계약·iOS 분리·옵션 사지선다를 반복 → 헌법 명문화로 미래 인스턴스 영구 차단. 8조 → 10조. v1.7.1·v1.8.0·v1.9.0·v2.0.0·v2.1.0 모두 FROZEN 보존, v2.2.0 신설. 코드 변경 0 (헌법 본문 변경만).
