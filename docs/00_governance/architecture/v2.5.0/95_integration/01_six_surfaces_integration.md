...
### 36-1-A2. v2.1.0 4-Layer + Real-time + Tag 다이어그램 (신규)
...
```
   ...
   │                  Data Layers (§30-3-A)                 │
   ├────────────────────────────────────────────────────────┤
   │  L1 OS (CallLog/SMS/Notif/PM/Contacts) — 영구 저장 X    │
   │  L2 MyPhoneCheck (라벨·태그§32·차단·휴지통·카드)         │
   │  L3 외부 캐시 (옵트인)                                   │
   │     ├─ A SecurityIntelligence (Abuse.ch/PhishTank)     │
   │     ├─ B GovernmentPublic    (KISA/FBI IC3)            │
   │     ├─ C CompetitorApp       (더콜·후후·Whoscall)        │
   │     └─ D TelcoBlocklist                                │
   │  L4 외부 검색 (Custom Tab, 사용자 trigger only)          │
   └────────────────────────────────────────────────────────┘
                │
                ▼
   ┌────────────────────────────────────────────────────────┐
   │   :core:global-engine                                  │
   │   parsing / search (4축) / decision/                   │
   │   ├─ InputAggregator   (L2·L1·L3 병렬, L4 인텐트)       │
   │   └─ RealTimeActionEngine §31  (BLOCK/SILENT/TAG/PASS) │
   └────────────────────────────────────────────────────────┘
                │
                ▼ (즉시 조치 + 결과 표시)
   ┌──────────┬──────────┬──────────┬──────────┬──────────┬──────────┐
   │CallScreen│SmsBlock  │PushTrash │MicCheck  │CameraChk │CardCheck │
   │  (§31-3) │ (§31-4)  │ (§31-5)  │          │          │          │
   └──────────┴──────────┴──────────┴──────────┴──────────┴──────────┘
                │
                ▼
...
핵심:
- **4-Layer** = 데이터 출처 분리 (§30-3-A).
- **검색 4축** = Internal·External·Public(Gov/Telco)·Public(Competitor) — §30-4.
- **RealTimeActionEngine** = L2 우선 즉시 BLOCK/SILENT, 50ms 응답 (§31).
- **Tag System** = L2 휘발성 메모, REMIND_ME/PENDING/SUSPICIOUS/ARCHIVE (§32).
