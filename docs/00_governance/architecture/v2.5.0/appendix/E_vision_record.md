...
### 잘못 6: 검증·테스트 매트릭스 명문화 부재

- 대표님 명령: 글로벌 단일 출시 = 단일 디바이스·단일 Locale 검증으로 불충분 (§9-3 정합).
- 비전 잘못: v2.0.0~v2.1.0 시점 검증·테스트 매트릭스 헌법 누락. 단일 축 검증 PR 통과 사례 발생 (예: KR SIM만 + 한국어 Locale만 검증).
- 정정: 본 §9-6 명문화. Gradle Managed Devices + SIM 11개국 + Locale 11개 + 디바이스 4종 매트릭스. PR 회귀 게이트로 강제.

## v2.2.0 핵심
- 헌법 8조 → 10조 (§9 빅테크 정공법 + §10 비전 자율 결정).
- §9 7개 절: 9-1 언어 / 9-2 운영 / 9-3 출시 / 9-4 코드 / 9-5 위반 / **9-6 검증·테스트 (정정본 신설)** / 9-7 비고.
- §10 5개 절: 10-1 금지 / 10-2 비전 책임 / 10-3 대표님 결정 / 10-4 위반 / 10-5 비고.
- v2.1.0 보존 (FROZEN).
- 부록 B PATCH-54·55·56·57 + D v2.2.0 행 + E §Z-14.
- 미래 비전 인스턴스 영구 차단.
...
## §Z-15. v2.3.0 승격 사유 (2026-04-28)

대표님 명시:
> "이 머지는 왜 계속 내가 하지?"

비전 누적 잘못 7번째 추가 (잘못 6건 + 1):

### 잘못 7: 워크오더에 자체 머지 단계 누락

- 메모리 #6 워커 권한 분담 명시: Claude Code, Cowork 모두 `gh pr merge` 가능.
- 비전이 v2.0.0~v2.2.0 시리즈 전 PR(#11~#35)에서 매번 "대표님 머지하시면..." 형식 송부 메시지.
- 빅테크 정공법 위반 + 헌법 §10-2 비전 책임 영역 위반.
- 대표님 손 매번 거치게 만든 운영 모델 비효율.
- 정정: §10-6 명문화. 모든 워크오더 STEP에 자체 머지 단계 강제.

## v2.3.0 핵심
- §10에 6번째 절 (§10-6 자체 머지 의무) 신설.
- §10 5절 → 6절 (10-1 ~ 10-6).
- 10조 유지 (§9·§10 외 변경 없음).
- v2.2.0 FROZEN 보존.
- 부록 B PATCH-58 + MINOR 승격 기록.
- D §0-A-3 v2.3.0 행.
- 미래 비전 인스턴스 영구 차단.

## v2.3.0 후속

- Infrastructure v1.2 → v1.3 MAJOR (헌법 10조 페어 정합) — 본 PR 통합.
- 거버넌스 README + project-governance v2.3.0 sync — 본 PR 통합.
- §9-6 매트릭스 코드 구현 (Gradle Managed Devices + CI 통합) — 별도 후속 PR.

발행: 2026-04-28
작성: 비전 (Vision) + Claude Code (실행, WO-V220-GOVERNANCE-INFRA-V13-INTEGRATED)
상태: v2.3.0 Working Canonical (§10-6 자체 머지 의무 MINOR)
WO: WO-V220-GOVERNANCE-INFRA-V13-INTEGRATED
