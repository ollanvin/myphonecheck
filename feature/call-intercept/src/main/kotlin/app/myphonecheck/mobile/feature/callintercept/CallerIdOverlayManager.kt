...
private const val MPC_OVERLAY = "MPC_OVERLAY"
...
class CallerIdOverlayManager @Inject constructor() {
    ...
    fun showOverlay(
        ...
        if (!canDrawOverlays(context)) {
            ...
            Log.w(MPC_OVERLAY, "BLOCKED permission=SYSTEM_ALERT_WINDOW number=$phoneNumber")
            return false
        }
        Log.i(MPC_OVERLAY, "SHOW number=$phoneNumber risk=${result.riskLevel}")
    ...
    private fun handleOverlayAction(context: Context, action: String, phoneNumber: String) {
        ...
        Log.i(MPC_ACTION, "BUTTON action=$action number=$phoneNumber latencyMs=$actionLatencyMs")
