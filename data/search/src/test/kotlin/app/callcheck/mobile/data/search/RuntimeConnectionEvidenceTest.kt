...
class RuntimeConnectionEvidenceTest {
    ...
    //
    // 코드 경로 증명:
    // CallCheckScreeningService.assessThenAllow() (line 150):
    ...
    // ═══════════════════════════════════════════════════════════
    //
    // 코드 증거 (CallCheckScreeningService.kt):
    //
    ...
    fun `EVIDENCE-3 PhoneNumberContextBuilder preserves raw number`() {
        ...
        val builder = app.callcheck.mobile.core.util.PhoneNumberContextBuilder()

        // 한국 번호 (하이픈 포함 raw)
        val rawKR = "010-1234-5678"
        val ctxKR = builder.build(rawKR, "KR", app.callcheck.mobile.core.model.NumberSourceContext.INCOMING_CALL)
        ...
        val ctxUS = builder.build(rawUS, "US", app.callcheck.mobile.core.model.NumberSourceContext.INCOMING_CALL)
        ...
        val ctxJP = builder.build(rawJP, "JP", app.callcheck.mobile.core.model.NumberSourceContext.INCOMING_CALL)
        ...
        println("\n[코드 경로 증거]")
        println("  CallCheckScreeningService.kt line 208: phoneNumber = phoneContext.rawNumber  ← UI에 raw 전달")
        println("  CallCheckScreeningService.kt line 189: normalizedNumber = canonicalNumber   ← 내부에 canonical 전달")
    ...
    fun `EVIDENCE-7 Device Relay is design-only and excluded from v1-0`() {
        ...
        val defaultMode = app.callcheck.mobile.core.model.RelayMode.OFF
        assertEquals("Default relay mode must be OFF", app.callcheck.mobile.core.model.RelayMode.OFF, defaultMode)

        // RelayPacket TTL 상수 존재 확인
        assertTrue("HIGH risk TTL must be 24h", app.callcheck.mobile.core.model.RelayPacket.TTL_HIGH_RISK == 24 * 60 * 60 * 1000L)
    }
}
